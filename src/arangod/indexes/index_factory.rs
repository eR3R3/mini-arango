...
impl VectorIndex {
    pub fn new(definition: IndexDefinition) -> Result<Self> {
        let dimensions = definition.vector_dimensions
            .ok_or_else(|| ArangoError::invalid_index_definition("Invalid vector dimensions"))? as usize;
...
impl IndexImplementation for VectorIndex {
    fn insert(&mut self, key: &[u8], document_ref: &[u8]) -> Result<()> {
        ...
        if key.len() % 4 != 0 {
            return Err(ArangoError::invalid_index_data("Invalid vector data"));
        }
        ...
        if vector.len() != self.dimensions {
            return Err(ArangoError::invalid_index_data(
