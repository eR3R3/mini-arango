...
impl RocksDBIndex {
    ...
    pub fn insert(&mut self, key: Vec<u8>, _document_ref: Vec<u8>) -> Result<()> {
        let key_len = key.len();
        
        // Update Bloom filter
        if let Some(ref mut bloom) = self.bloom_filter {
            let hash = Self::calculate_simple_hash(&key);
            let bucket = (hash % bloom.len() as u64) as usize;
            bloom[bucket] = 1; // Set bit to 1 instead of true
        }
        
        // Update statistics
        let mut stats = self.statistics.write().unwrap();
        stats.size += 1; // Use size instead of entries
        stats.memory_usage += key_len as u64 + 8; // rough estimate
        
        Ok(())
    }
    
    /// Calculate hash for a key (static method to avoid borrowing issues)
    fn calculate_simple_hash(key: &[u8]) -> u64 {
        let mut hash: u64 = 0;
        for &byte in key {
