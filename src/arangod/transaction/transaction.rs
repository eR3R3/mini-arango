            ...
            match func(&mut txn_context) {
                Ok(result) => {
                    match txn_context.commit() {
                        ...
                        Err(e) if e.error_code() == crate::common::error::ErrorCode::TransactionConflict && retry_count < options.max_retries => {
                            retry_count += 1;
                            std::thread::sleep(std::time::Duration::from_millis(
                                options.retry_delay_ms * (2_u64.pow(retry_count as u32 - 1))
                            ));
                            continue;
                        },
                        Err(e) => return Err(e),
                ...
                Err(e) => {
                    ...
                    if e.error_code() == crate::common::error::ErrorCode::TransactionConflict && retry_count < options.max_retries {
                        retry_count += 1;
                        println!("Transaction error, retrying ({}/{}): {}", retry_count, options.max_retries, e);
                        
                        let delay = Duration::from_millis(options.retry_delay_ms * (2_u64.pow(retry_count as u32)));
