...
impl GeoPoint {
    pub fn new(lat: f64, lng: f64) -> Result<Self> {
        if lat < -90.0 || lat > 90.0 {
            return Err(ArangoError::InvalidGeoCoordinate(format!("Invalid latitude: {}", lat)));
        }
        if lng < -180.0 || lng > 180.0 {
            return Err(ArangoError::InvalidGeoCoordinate(format!("Invalid longitude: {}", lng)));
        }
...
impl BoundingBox {
    pub fn new(min_lat: f64, min_lng: f64, max_lat: f64, max_lng: f64) -> Result<Self> {
        if min_lat > max_lat || min_lng > max_lng {
            return Err(ArangoError::InvalidGeoCoordinate("Invalid bounding box".to_string()));
        }
